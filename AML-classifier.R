...
# Classification method development (LR, SVM, RF) developed in python with sklearn
# Python code file: AML-classifier.ipynb
# RF model was chosen as final model 
# The following code only focuses on random forest
...
length(data2Test.pred)
table(observed = data2Test$Class, predicted = data2Test.pred)

subdata2Train <- data2Train[, c(106:140, 281)]
subdata2 <- data2[, c(106:140, 281)]
write.table(subdata2Train, "subdata2Train.txt")


model <- svm(as.factor(Class) ~ ., subdata2Train)
predictedY <- predict(model,  data2Test)
table(observed = data2Test$Class, predicted = predictedY)
...
output_dir <- "Users/wz/spade_output"
...
###################################################
### code chunk number 6: SPADE.Rnw:123-124
###################################################
...
set.seed(415)
fit3 <- randomForest(as.factor(Label) ~ .,
                     data=subdata2Train, 
                     importance=TRUE, 
                     ntree=1000)
fit3
subdata2Test.pred <- predict(fit3, data2Test)
table(observed = data2Test$Label, predicted = subdata2Test.pred)
...
library(psych)
library(flowCore)
expr1.1 <- as.data.frame(exprs(read.FCS("0006.FCS", transformation="scale", alter.names=TRUE)))
...
pred1 <-predict(fit3, data2Test[1, c(176:210, 281)], "prob")
pred1 <-predict(fit3, data2Test[1, c(176:210, 281)])
as.character(pred1)
...
Normal_1 <- as.data.frame(exprs(read.FCS("~/Downloads/FlowCAP-II/Data/AML/FCS/0796.FCS", transformation="scale", alter.names=TRUE)))

healthy1<-read.FCS("~/Downloads/FlowCAP-II/Data/AML/FCS/0004.FCS",  alter.names=TRUE)
aml1<-read.FCS("~/Downloads/FlowCAP-II/Data/AML/FCS/0036.FCS", alter.names=TRUE)
...
head(pData(GvHD))
GvHD
xyplot(`FS.Lin` ~ `FL2.Log`, data=fcs, smooth=FALSE)
densityplot(~ `FL4.log`, data=healthy1, xlim=c(0,1000))
densityplot(~ `FL4.log`, data=aml1, xlim=c(0,1000))
...
library(plotrix)
pie3d(slices,labels=lbls,explode=0.1, radius=0.75, start=0)
